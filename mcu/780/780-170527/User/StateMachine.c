...
BOOL SetCurrentStateValue(EventValue Val,State currentState,U16 wValue)
{
	...
	if(Val==KEY_ENTER)
	{
		...
		case State_FactoryReset_OK:
			...
			GpuSend("SBC(7);CBOF(18,60,200,130,0,7);DS16(18,82,' 正在恢复出厂，请稍等...',0);\r\n");
			//delay_ms(40);
			for (i=0;i<RESET_WAITING_TIME;i++)
				{
				sprintf(GpuBuf,"CBOF(23,146,%d,158,0,2);\r\n",i*3);
				GpuSend(GpuBuf);
				delay_ms(1);
				}
			...
			if(ResetPara_Total())
			{
					for(led_no=0;led_no<KEY_NUM;led_no++)
					{
						...
						//delay1000(1000);
						delay_ms(10);//===2017,Jun 17 ; 14:26:10  ;BY YANG@BYCOX.COM===
					}	
			}	
