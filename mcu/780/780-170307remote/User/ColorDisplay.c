		...
		{	
			if(LCD_UI_NO != 13)
			{
				...
				if(g_ImageNum_Set==1 &&g_ImageNum==1)
				{
					...
					g_input_imagepara[2]=d_screen_total_X;
					g_input_imagepara[3]=d_screen_total_Y;
					GpuSend("SBC(7);");	
					sprintf(GpuBuf,"DS16(68,63,'  %d',0);",g_input_imagepara[0]);
					GpuSend(GpuBuf);
					GpuSend("SBC(7);");	
					sprintf(GpuBuf,"DS16(170,63,' %d',0);",g_input_imagepara[1]);
					GpuSend(GpuBuf);
					GpuSend("SBC(7);");	
					sprintf(GpuBuf,"DS16(68,93,'%d',0);",d_screen_total_X);
					GpuSend(GpuBuf);
					GpuSend("SBC(7);");	
		...
		case State_FactoryReset_OK:
		{
			...
			if(g_CurrentState==State_FactoryReset_Yes)
			...
			
				{
					...
					GpuSend("SBC(7);DS12(47,72,'该操作将清除保存数据!',0);\r\n");
				}
				delay_ms(120);
	...
	switch(CurState)
	{
		...
		//显示总高和宽
			...
			for(i=0;i<g_LineNum;i++)
			  total+=g_screen_line_column_pixel_input[1][i];
			if(total>9999)
			...
			for(i=0;i<g_ColNum;i++)
			  total+=g_screen_line_column_pixel_input[0][i];
			if(total>9999)
		...
		case State_MainMenu_Menu_InOutput:
			...
			input_source_resolution[0].resolution_width,input_source_resolution[0].resolution_high,
			input_source_resolution[1].resolution_width,input_source_resolution[1].resolution_high,
			input_source_resolution[2].resolution_width,input_source_resolution[2].resolution_high,
			input_source_resolution[3].resolution_width,input_source_resolution[3].resolution_high);
			GpuSend(GpuBuf);
				delay_ms(80);
			LCD_UI_NO=101;
			break;
...
实现了读取场景，并进行显示
******************************/
void ColorDisplayCurrentScenario()
{
...
ColorDisplaySoftRevision(U8 SoftVersion[])//+++++++++++++++++++++++++++++
{
	char GpuBuf[100];
...
ColorDisMachineIDValue(U8 MachineIDValue[])//+++++++++++++++++++++++++++++
{
	char GpuBuf[100];
...
ColorDisMachineRevision(U8 MachineRevision[])//+++++++++++++++++++++++++++++
{
	char GpuBuf[100];
...
ColorDisPhoneNumber(U8 PhoneNumber[])//+++++++++++++++++++++++++++++
{
	char GpuBuf[100];
...
//					break;	
		...
		case State_ScreenSet_FourPort_layout3:	
			if(g_LineNum_Set<=g_LineNum)
			{
				...
				GpuSend(GpuBuf);
				g_screen_line_column_pixel_input[1][g_LineNum_Set-1]=1080;
				sprintf(GpuBuf,"SBC(15);CBOF(131,126,204,149,1,15);DS16(140,130,'%d',7);\r\n",g_screen_line_column_pixel_input[1][g_LineNum_Set-1]);
				GpuSend(GpuBuf);
			}
			else
			{
				...
				GpuSend(GpuBuf);
				g_screen_line_column_pixel_input[0][g_ColNum_Set-1]=1920;
				sprintf(GpuBuf,"SBC(15);CBOF(131,126,204,149,1,15);DS16(140,130,'%d',7);\r\n",g_screen_line_column_pixel_input[0][g_ColNum_Set-1]);
				GpuSend(GpuBuf);
			}
			break;		
		case State_ScenarioSet_NewImage_ImagePara_Source:
		case State_WizardSet_InputSource_Set:
		{
				...
				GpuSend(GpuBuf);
				if(input_source_resolution[0].resolution_width!=0)
				{
					GpuSend("SBC(5);");
				...
				if(input_source_resolution[1].resolution_width!=0)
				{
					GpuSend("SBC(5);");
				...
				if(input_source_resolution[2].resolution_width!=0)
				{
					GpuSend("SBC(5);");
				...
				if(input_source_resolution[3].resolution_width!=0)
				{
					GpuSend("SBC(5);");
		...
		case State_AdvanceSet_CardTest_Port:
		{
			...
			GpuSend("SBC(4);");		
			GpuSend("CBOF(10,60,80,87,7,4);CBOX(10,60,80,87,7,0);DS12(22,68,'输出端口',0);\r\n");
			
			GpuSend("CBOF(100,60,200,90,0,7);CBOX(100,60,200,90,0,0);");
			GpuSend("PL(125,60,125,90,0);PL(150,60,150,90,0);PL(175,60,175,90,0);\r\n");
			GpuSend("SBC(7);");		
			GpuSend("DS16(109,69,'1',0);DS16(134,69,'2',0);DS16(159,69,'3',0);DS16(184,69,'4',0);\r\n");

			GpuSend("SBC(5);");	
			switch(g_CardTest_Port)
			{
				case 1:
					GpuSend("CBOF(101,61,124,89,0,5);CBOX(101,61,124,89,0,7);DS16(109,69,'1',0);\r\n");
					break;
				case 2:
					GpuSend("CBOF(126,61,149,89,0,5);CBOX(126,61,149,89,0,7);DS16(134,69,'2',0);\r\n");
					break;
				case 3:
					GpuSend("CBOF(151,61,174,89,0,5);CBOX(101,61,174,89,0,7);DS16(159,69,'3',0);\r\n");
					break;
				case 4:
					GpuSend("CBOF(176,61,199,89,0,5);CBOX(176,61,199,89,0,7);DS16(184,69,'4',0);\r\n");
					break;
				default:
					break;				
			}			
