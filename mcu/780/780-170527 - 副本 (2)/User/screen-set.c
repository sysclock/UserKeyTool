...
void Matrix_Display_Function(void)
{
	...
	//¾ØÕóÄ£Ê½ÊýÖµ¼ÆËã
	if(gMatOutpNo==0)  //ËùÓÐµÄ¶¼Êä³ö
	{
		//case 1: 0 vs 0
		if(gMatInpNo==0) //0v0 Ò»¶ÔÒ»Æ¥Åä1-->1;2-->2;3-->3;4-->4
		{
		...
		//case 2: n vs 0;1-->all/multi
		else	
		{
//				n vs m,Work Normally
//				if(InSrcRes[gMatInpNo-1].resolution_width!=0)//ÓÐÐÅºÅÊäÈë£¬ÊäÈë·ç±æÂÊ²»Îª0
//				{						
...
//				}	
			...
			printf("\r\n enter n vs 0 module of matrix_display funciton in screen-set.c file \r\n");
			#endif
			for(i=0;i<IN_PORT_NUM;i++)
			{	
				gMatImgPara[i][LAYER_INDEX]=0;
				...
				{					
					...
					gMatImgPara[i][WIDTH_INDEX]=1920;
					gMatImgPara[i][HIGH_INDEX]=1080;
					gMatImgPara[i][SOURCE_IN_INDEX]=gMatInpNo-1;//is diff with if(gMatInpNo==0)
					gMatImgPara[i][LAYER_INDEX]=i+1;
					gMatImgPara[i][DEAL_INDEX]=0;		
				}
			}	//end for
	...
	else// -vm Êä³ö¿Ú²»µÈÓÚ0£»²»ÊÇ¶¼Êä³ö
	{
		//case 3: 0 vs m;all/multi --> 1 
		if(gMatInpNo==0) //0vm   ¶à»­ÃæÄ£Ê½?°ÑËùÓÐµÄÊä³ö¶¼Êä³öµ½Ä³¸ö¿Ú
		{
				gMatImgPara[i][LAYER_INDEX]=0;
				if(InSrcRes[gMatOutpNo-1].resolution_width!=0)
				{					
					gMatImgPara[gMatOutpNo-1][BEGIN_X_INDEX]	=(gMatOutpNo-1)*1920;
					gMatImgPara[gMatOutpNo-1][BEGIN_Y_INDEX]	=0;
					gMatImgPara[gMatOutpNo-1][END_X_INDEX]	=gMatImgPara[gMatOutpNo-1][0]+1920;
		...
		//case 4: n vs m;p2p;  work normally 
		else
		{	
				gMatImgPara[i][LAYER_INDEX]=0;
				if(InSrcRes[gMatInpNo-1].resolution_width!=0)//ÓÐÐÅºÅÊäÈë£¬ÊäÈë·ç±æÂÊ²»Îª0
	...
	for(i=0;i<IN_PORT_NUM;i++)
	{
		//gImgPara[i][LAYER_INDEX]=0;//Õâ¸ö²ÎÊý²»ÎªÁã£¬±íÊ¾ÓÐÐÅºÅÔ´
	...
	//gImgParaÊý¾Ý×ª»¯ÎªËõ·ÅÆ÷µÈ²ÎÊý
	image_layout_Protocal_Prara_Process();
}
