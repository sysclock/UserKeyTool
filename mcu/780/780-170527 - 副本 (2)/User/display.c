...
//the resolution SET of ALL input sources/ports 
...
void save_scenario_to_FPGA(u8 num)
{
	sprintf(gCmdBuf,"GP SDMI %d\r\n",num);
	Uart_Send_Cmd_to_FPGA(gCmdBuf);
	Output_Set_Protocal_Cmd_Send2FPGA();
	GpuSend("SBC(7);");
...
void InFPGA_Scaler_Para_Process(u8 num)
{
	...
	for(i=line;i<=line_end;i++)
	{
		...
		for(j=col;j<=col_end;j++)
		{
			...
			AW_BG_No=InFPGA_AddWin_Para_Process(inBoardUsedScalerNum,1);
...
void image_layout_Protocal_Prara_Process(void)
{
	...
	#ifdef DEBUG88
	printf("enter image layout 3p function of display.c line 1662 \r\n");

	printf("LAYER-IDX:  1=%d \r\n",gImgPara[0][LAYER_INDEX]);

	
	printf("LAYER-IDX:  1=%d,2=%d,3=%d,4=%dn,5=%d,6=%d,7=%d,8=%d \r\n"
	...
	for(i=0;i<IN_PORT_NUM;i++) //´¦ÀíÍ¼²ãÊý¾Ý
	{
		...
		{
//			case 1:width & high not both equal
			if(gImgPara[i][WIDTH_INDEX]<InSrcRes[gImgPara[i][SOURCE_IN_INDEX]].resolution_width 
				&& gImgPara[i][HIGH_INDEX]<=InSrcRes[gImgPara[i][SOURCE_IN_INDEX]].resolution_high
				
				||gImgPara[i][WIDTH_INDEX]<=InSrcRes[gImgPara[i][SOURCE_IN_INDEX]].resolution_width 
				&& gImgPara[i][HIGH_INDEX]<InSrcRes[gImgPara[i][SOURCE_IN_INDEX]].resolution_high)  //µ±Ç°Í¼Ïñ²ãÍ¼Ïñ³ß´çÐ¡ÓÚ
			{
						...
						if(AW_BG_No>0)  //Ö±½Ó½«ÐÅºÅ·¢ËÍÖÁÊä³ö°å½øÐÐ´¦Àí ,Èç¹ûÐÅºÅÎÞ·¨Í¨¹ý²¼¾ÖÆ÷ËÍÈëµ½Êä³ö°å£¬¾Í·ÅÆúÍ¼Ïñ
						{
							...
							{		
								...
								{											
									...
									OF_Scaler_Para[inBoardUsedScalerNum][2]=AW_BG_No;//ÊäÈëÐÅºÅÔ´
									///////////////////////////////////////
									//ÒÔÏÂµÄ²ÎÊýÓ¦¸ÃÊÇ¸ù¾ÝÊµ¼ÊÊäÈëÔ´µÄ²ÎÊý½øÐÐ¸³Öµ
...
//			case 2:width or high larger than
			else if(gImgPara[i][WIDTH_INDEX]>InSrcRes[gImgPara[i][SOURCE_IN_INDEX]].resolution_width
				|| gImgPara[i][HIGH_INDEX]>InSrcRes[gImgPara[i][SOURCE_IN_INDEX]].resolution_high) //ÐèÒª·Å´ó
			{					
...
//			case 3 :width & high both equal
			else //Í¼ÏñÊä³öÓëÊäÈëÒ»Ñù£¬²»ÓÃ·ÖÅäËõ·ÅÆ÷£¬Ö±½Ó´«ÈëÊä³ö°å½øÐÐ·Ö¾ÖÏÔÊ¾
			{
				...
				if(AW_BG_No>0)  //Ö±½Ó½«ÐÅºÅ·¢ËÍÖÁÊä³ö°å½øÐÐ´¦Àí ,Èç¹ûÐÅºÅÎÞ·¨Í¨¹ý²¼¾ÖÆ÷ËÍÈëµ½Êä³ö°å
				{					
					input_to_output_layout(AW_BG_No,source_to_outboard_num,i);										
				}		
			}
