...
static void SetSwitchFM(u8 Input,u8 Output)
{
	...
	if((InOut_V[Output-1]&0x80)&&(((InOut_V[Output-1]&0x7F)+1) != Input))
	{
	    Board = (InOut_V[Output-1]&0x7f) >> 3;	
            ...
            if(Output%2)
            {
              if(InOut_V[Output]&0x80)
              {
                  Data = ((InOut_V[Output]&0x7F)+1)<<4;
              }
              else
              {
                Data = 0;
            ...
            {
               if(InOut_V[Output-2]&0x80)
               {
                  Data = ((InOut_V[Output-2]&0x7F)+1)&0xf;                 
               }
               else
               {
                 Data = 0;
...
void InitFm()
{
	...
	{
		SetVAddr(i);
		FMCtrl(0xFF,0x3);//Clamp    //FMCtrl(0xFF,0x1d);
		FMCtrl(0x0,0x4);//GAIN 6db    //FMCtrl(0xFF,0x1e);
		SetVAddr(0x0f);		
	}
}
