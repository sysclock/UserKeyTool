...
BOOL ReadLog()
{
	...
	while(num<=(8192-E2ROM_Log_Start_ADDR)/5)
	{
		...
		printf("  \n%d年%d月%d日%d:%d 开机%d时%d分",(cch[0]>>4&0x0F)+2013,cch[0]&0x0F,cch[1]>>3&0x1F,
...
void DealStartTime()
{
	...
	{//从输入开机密码后开始计时
		...
		if(MinuteTime>=g_howlongtosave&&(MinuteTime-g_howlongtosave)%5==0&&second==0&&MinuteTime<0x10000)
		{
			if(once==1)
			{
				once=2;
