		...
		if(BackupInput==INPUTSOURCE_NONE)
		{
			switch(input)
			{
				case INPUTSOURCE_1DVI:
					BackupInput=INPUTSOURCE_2DVI;
					break;
				case INPUTSOURCE_2DVI:
					BackupInput=INPUTSOURCE_3DVI;
					break;
				case INPUTSOURCE_3DVI:
					BackupInput=INPUTSOURCE_4DVI;
					break;
				case INPUTSOURCE_4DVI:
					BackupInput=INPUTSOURCE_1VGA;
					break;
				case INPUTSOURCE_1VGA:
					BackupInput=INPUTSOURCE_2VGA;
					break;
				case INPUTSOURCE_2VGA:
					BackupInput=INPUTSOURCE_3VGA;
					break;
				case INPUTSOURCE_3VGA:
					BackupInput=INPUTSOURCE_4VGA;
					break;
			
				case INPUTSOURCE_4VGA:
					BackupInput=INPUTSOURCE_1CVBS;
					break;
				case INPUTSOURCE_1CVBS:
					BackupInput=INPUTSOURCE_2CVBS;	
					break;
				case INPUTSOURCE_2CVBS:
					BackupInput=INPUTSOURCE_1DVI;
					break;
				default:
					break;	
...
void Detect_Current_Signal()
{
	if(g_EnableHotBackupFlag&&DetectNoSignalImage(&g_ActiveImageValue))
	{
		...
		else//表示32626开机默认的DVI 没信号
		{
			...
			if(g_MainInput==INPUTSOURCE_1DVI)  //刚开机判断当前信号是不是DVI
				SwitchBackup(g_ActiveImageValue);
...
{	
	...
	GPIO_InitStructure.GPIO_Pin =GPIO_Pin_7;   //启动工控机  
...
U8 gB_SerialMachineAddress = 0;

U8	gB_SerialCommadIndex=0;
U8	gB_SerialCommadBuf[SERIAL_COMMAD_BUF_LENGTH];





U8 ParseSerialCommadBySelfType(U8 B_Data)
{
	U8 B_Temp = NULL;
	B_Temp = B_Data;
...
{
	U8 B_Temp = 0, B_SerialSelfCommandNum = 0, ret = 0;
	while(QueueOut(&g_strUartRxQueue, &B_Temp)!= QueueEmpty)
	{
...
char Usart3_Rev_Buf[128];
char Usart3_Rev_Idx=0;
...
void USART3_IRQHandler(void)
{
	...
	{	 
		...
		if(Usart3_Rev_Idx<127)
		{
			Usart3_Rev_Buf[Usart3_Rev_Idx]=res;		//记录接收到的值
			Usart3_Rev_Idx++;						//接收数据增加1 
		} 
		else
		  Usart3_Rev_Idx=0;
	}  
