...
void HDMITX_ChangeDisplayOption(HDMI_Video_Type OutputVideoTiming, HDMI_OutputColorMode OutputColorMode)
{
   //HDMI_Video_Type  t=HDMI_480i60_16x9;
    ...
    {
        OutputColorMode=(HDMI_OutputColorMode) F_MODE_RGB444;
    }
    else if ((F_MODE_YUV422)==(bOutputColorMode&F_MODE_CLRMOD_MASK))//YUV422 only
    {
        if(OutputColorMode==HDMI_YUV444){OutputColorMode=(HDMI_OutputColorMode) F_MODE_YUV422;}
    }
    else if ((F_MODE_YUV444)==(bOutputColorMode&F_MODE_CLRMOD_MASK))//YUV444 only
    {
        if(OutputColorMode==HDMI_YUV422){OutputColorMode=(HDMI_OutputColorMode) F_MODE_YUV444;}
    }
...
BYTE ParseEDID()
{
	...
	if( CheckSum != 0 )
	{
		EDID_DEBUG_PRINTF(("EDID Check Sum Error!\r\n"));
		return FALSE ;
	}
	if( EDID_Buf[0] != 0x00 ||
	    EDID_Buf[1] != 0xFF ||
	    ...
	    EDID_Buf[7] != 0x00)
    {
				EDID_DEBUG_PRINTF(("EDID Packet Head Error!\r\n"));
        return FALSE ;
    }
    ...
    for( i = 0 ; i < 128 ; i++ )
    {
        HDMITX_DEBUG_PRINTF(("%02X%s",(int)EDID_Buf[i],(7 == (i&7))?"\r\n" :" "));
    }
		EDID_DEBUG_PRINTF(("\r\n"));
		#endif
    ...
    for( i = 1 ; i <= BlockCount ; i++ )
    {
        ...
        for( j = 0 ; j < 128 ; j++ )
        {
            EDID_DEBUG_PRINTF(("%02X%s",(int)EDID_Buf[j],(7 == (j&7))? "\r\n" :" "));
        }
				EDID_DEBUG_PRINTF(("\r\n"));
        #endif // Debug_message
...
static BOOL ParseCEAEDID(BYTE *pCEAEDID)
{
    ...
    for( offset = 4 ; offset < End ; )
    {
        ...
        case 0x03: // Vendor Specific Data Block ;
            ...
            EDID_DEBUG_PRINTF(("IEEEOUI = %02X %02X %02X %lx\r\n",(int)pCEAEDID[offset+2],(int)pCEAEDID[offset+1],(int)pCEAEDID[offset],RxCapability.IEEEOUI));
			if( RxCapability.IEEEOUI== 0x0C03)
			{
				BYTE nextoffset ;
