...
const  CECcmdFormat  LookUpTable_CEC[]=
{
    ...
    // Polling Mesage
    //ePollingMessage,
    {(eCEC_CMD) (eCDC+1),                eBoth,      0},         //end of     lookup table !!!
};
...
BYTE cec_cnt100ms(void)
{
    ...
    //for(T100us=0;T100us<10;T100us++)
    {
        ...
        tick=gD_IT66121TickCnt;
        CEC_SetI2C_Byte( REG09, B_EN100ms_CNT, 0 );
        tick=gD_IT66121TickCnt-tick;
        tick = 100 - tick;
...
void HDMITX_CEC_Init(void)
{
    ...
    BYTE u8_LogicAddress=DEVICE_ID_AUDIO;
#else
    #ifdef  _CEC_DEVICE_PLAYBACK_
        BYTE u8_LogicAddress=DEVICE_ID_PLAYBACK1;
    #else
        BYTE u8_LogicAddress=DEVICE_ID_TV;
    #endif
#endif
...
#ifdef DEBUG_iT6601
            IT6601_PRINT(("PollingMessage Header=%X \r\n",TxCmdBuf.HEADER));
#endif
            TxCmdBuf.SIZE=1;
            break;
...
#ifdef DEBUG_iT6601
    printf("CECCmdSet --> Tx Header:%X, opcode:%X , operand1:%X , operand2:%X ,Size:%X-----\r\n ",
           TxCmdBuf.HEADER,
           TxCmdBuf.OPCODE,
           TxCmdBuf.OPERAND1,
           TxCmdBuf.OPERAND2,
...
void CECDecoder(void)
{
    ...
    switch ( pCECRxBuf->SIZE )
    {
        default:
        case 18:
            IT6601_PRINT(("OPERAND16=%X \r\n",pCECRxBuf->OPERAND16));
        case 17:
            IT6601_PRINT(("OPERAND15=%X \r\n",pCECRxBuf->OPERAND15));
        case 16:
            IT6601_PRINT(("OPERAND14=%X \r\n",pCECRxBuf->OPERAND14));
        case 15:
            IT6601_PRINT(("OPERAND13=%X \r\n",pCECRxBuf->OPERAND13));
        case 14:
            IT6601_PRINT(("OPERAND12=%X \r\n",pCECRxBuf->OPERAND12));
        case 13:
            IT6601_PRINT(("OPERAND11=%X \r\n",pCECRxBuf->OPERAND11));
        case 12:
            IT6601_PRINT(("OPERAND10=%X \r\n",pCECRxBuf->OPERAND10));
        case 11:
            IT6601_PRINT(("OPERAND9=%X \r\n",pCECRxBuf->OPERAND9));
        case 10:
            IT6601_PRINT(("OPERAND8=%X \r\n",pCECRxBuf->OPERAND8));
        case 9:
            IT6601_PRINT(("OPERAND7=%X \r\n",pCECRxBuf->OPERAND7));
        case 8:
            IT6601_PRINT(("OPERAND6=%X \r\n",pCECRxBuf->OPERAND6));
        case 7:
            IT6601_PRINT(("OPERAND5=%X \r\n",pCECRxBuf->OPERAND5));
        case 6:
            IT6601_PRINT(("OPERAND4=%X \r\n",pCECRxBuf->OPERAND4));
        case 5:
            IT6601_PRINT(("OPERAND3=%X \r\n",pCECRxBuf->OPERAND3));
        case 4:
            IT6601_PRINT(("OPERAND2=%X \r\n",pCECRxBuf->OPERAND2));
        case 3:
            IT6601_PRINT(("OPERAND1=%X \r\n",pCECRxBuf->OPERAND1));
        case 2:
            IT6601_PRINT(("OPCODE=%X \r\n",pCECRxBuf->OPCODE));
        case 1:
            IT6601_PRINT(("HEADER=%X \r\n",pCECRxBuf->HEADER));
        case 0:
            IT6601_PRINT(("SIZE =%X \r\n",pCECRxBuf->SIZE));
            break;
    }
    ...
    IT6601_PRINT(("SizeOfLookUpTable_CEC=%X \r\n", index));
        ...
        case eInactiveSource :      //<MS> <0x9D> <Physical Address>        example:<0x40> <0x9D> <AB> <CD>
            ...
            if ( Myself_LogicAdr==DEVICE_ID_TV )   // for TV only !!!
            {
                ...
                //f(Myself_LogicAdr==uc)
                {
                    ...
                    {
                        IT6601_PRINT(("CECList[%X].Active -> FALSE\r\n",uc));
...
//=== 13.6 System Information ===
        case eCECVersioin :     //<MS> <0x9E> <CEC version>
            IT6601_PRINT(("Receive !!!CEC Version =%X!!!\r\n",pCECRxBuf->OPERAND1));
            break;
        case eGetCECVersion :       //<MS> <0x9F> , example: [0x05 0x9F] TV request AVR to reply CEC version
            IT6601_PRINT(("Receive !!!Report CEC Version !!!\r\n"));
            uc=(pCECRxBuf->HEADER & 0xf0)>>4;
// HL            P1_6=1;
// HL            CECCmdSet(uc, eCECVersioin,((P1_6==1)?0x04:0x5),0); // 5 for Version 1.4a... 4 for Version 1.3A...0~3 for Reserved
...
{
    IT6601_PRINT(("Switch_OneTouchPlayState--> %X \r\n",uState));
...
//        Myself_LogicAdr==DEVICE_ID_PLAYBACK2 || Myself_LogicAdr==DEVICE_ID_PLAYBACK3  ||)
    {
        IT6601_PRINT(("switch_PollingMessage--> %X \r\n",state));
        switch ( state )
        {
...
// for iT6633+iT6623{
// for iT6633+iT6623    IT6601_PRINT(("Switch_LogicalAddressingState--> %X \r\n",uState));
        ...
        case sPollingMessage:
            {
                ...
                else
                {
                    ...
                    for ( uc=1;uc<15;uc++ )
                    {
                        IT6601_PRINT(("CECList[%X]= %X , PhysicalAddr=[%X][%X] \r\n",
                                      uc,
                                      CECList[uc].Active,
        ...
        case    sHEC_InquirState:
            {
                ...
                if ( pCECRxBuf->OPERAND3==eHEC_InquirState )
                {
                    ...
                    //FollowerAddr=uc;
                    //pCECRxBuf->OPERAND1
                    IT6601_PRINT(("-----Header=%X, opcode=%X , operand1=%X , operand2=%X ,FollowerAddr=%X-----\r\n ",
                                  pCECRxBuf->HEADER,
                                  pCECRxBuf->OPCODE,
                                  pCECRxBuf->OPERAND1,
                                  pCECRxBuf->OPERAND2,
...
// for iT6633+iT6623            IT6601_PRINT(("===CEC Active Port = %X ===\r\n",i));
// for iT6633+iT6623
...
{
    IT6601_PRINT(("!!! SwitchSystemInfoFeature -> %X !!!\r\n",State));
    SACState=State;
    switch ( SACState )
    {
        ...
        case sSetSystemAudioModeToTV:{
                if ( OldTxCECcmd==eSetSystemAudioMode )
                {
                    switch ( CECTxState )
                    {
                        ...
                        case sCECReceiverNack:
                            IT6601_PRINT(("!!! Nack !!!\r\n"));
                            if ( TimerCheck(IT_66121_CEC_100ms_TMR)==TIMER_TMO )
                                Switch_SystemAudioControl(sSetSystemAudioModeToAll);
                            break;
                        case sCECOk:
                        case sCECNone:
...
// for iT6633+iT6623                        printf("CEC volume = %X \r\n",uCECAudioStatus.Map.AudioVolumeStatus);
            }
            else
            {
                if ( (uCECAudioStatus.Map.AudioVolumeStatus) == 0x00 )
