...
*/
			i = 0;
			while (I2C0WriteByte(address, offset, I2C_DEV_MEMORY_ADDRESS_8BIT, byteno, p_data) == (BYTE) ERR)
			{
				Delay10ms(1);
				i++;
				if(i == 5)
				{
					#if(TVP5158_DEBUG == 1)
					printf("IT66121 0x%02X, I2CDEV 0x%02X write Data ERROR!\r\n", device, address);
					#endif
					return 0;
				}
			}
...
*/
			if ((p_Tempdata = I2C0ReadByte(address, offset, I2C_DEV_MEMORY_ADDRESS_8BIT, byteno)) == NULL)
			{
				Delay10ms(1);
				if((p_Tempdata = I2C0ReadByte(address, offset, I2C_DEV_MEMORY_ADDRESS_8BIT, byteno)) == NULL)
				{
					#if(TVP5158_DEBUG == 1)
					printf("IT66121 0x%02X, I2CDEV 0x%02X read ERROR!\r\n", device, address);
					#endif
					return 0;
				}
			}
			memcpy(p_data, p_Tempdata, byteno);
			return 1;
}
