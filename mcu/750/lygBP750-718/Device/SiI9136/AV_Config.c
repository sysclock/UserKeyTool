...
bool InitVideo(byte TclkSel)
{
	...
	TPI_DEBUG_PRINT(("InitVideo: VIC:%d\r\n", (int) VideoModeDescription.VIC));
	TPI_DEBUG_PRINT(("AspectRatio:%x\r\n", (int) VideoModeDescription.AspectRatio));
	TPI_DEBUG_PRINT(("ColorSpace:%x\r\n", (int) VideoModeDescription.ColorSpace));
	TPI_DEBUG_PRINT(("ColorDepth:%x\r\n", (int) VideoModeDescription.ColorDepth));
	TPI_DEBUG_PRINT(("HDCPAuthenticated:%d\r\n", (int) VideoModeDescription.HDCPAuthenticated));
	TPI_DEBUG_PRINT(("HDMIVideoFormat:%d\r\n", (int) VideoModeDescription.HDMIVideoFormat));
	TPI_DEBUG_PRINT(("3D:%d\r\n", (int) VideoModeDescription.ThreeDStructure));
	TPI_DEBUG_PRINT(("3Dx:%d\r\n", (int) VideoModeDescription.ThreeDExtData));

	ModeTblIndex = (byte)ConvertVIC_To_VM_Index();
//	ModeTblIndex = 24;
	TPI_DEBUG_PRINT (("ModeTblIndex = %d\r\n", ModeTblIndex));
	...
	TPI_DEBUG_PRINT(("PixClk:%d\r\n", VModesTable[ModeTblIndex].PixClk));
	TPI_DEBUG_PRINT(("VFreq:%d\r\n", VModesTable[ModeTblIndex].Tag.VFreq));
	TPI_DEBUG_PRINT(("Total.Pixels:%d\r\n", VModesTable[ModeTblIndex].Tag.Total.Pixels));
	TPI_DEBUG_PRINT(("Total.Lines:%d\r\n", VModesTable[ModeTblIndex].Tag.Total.Lines));
	#else
	// Test 3840x540
	// HL Add
	B_Data[4] = (VModesTable[ModeTblIndex].Tag.Total.Pixels << 1) & 0x00FF;	// write total number of pixels to TPI registers 0x04, 0x05
	B_Data[5] = ((VModesTable[ModeTblIndex].Tag.Total.Pixels << 1) >> 8) & 0xFF;

	B_Data[6] = (VModesTable[ModeTblIndex].Tag.Total.Lines >> 1) & 0x00FF;	// write total number of lines to TPI registers 0x06, 0x07
	B_Data[7] = ((VModesTable[ModeTblIndex].Tag.Total.Lines >> 1) >> 8) & 0xFF;
	TPI_DEBUG_PRINT(("PixClk:%d\r\n", VModesTable[ModeTblIndex].PixClk));
	TPI_DEBUG_PRINT(("VFreq:%d\r\n", VModesTable[ModeTblIndex].Tag.VFreq));
	TPI_DEBUG_PRINT(("Total.Pixels:%d\r\n", VModesTable[ModeTblIndex].Tag.Total.Pixels << 1));
	TPI_DEBUG_PRINT(("Total.Lines:%d\r\n", VModesTable[ModeTblIndex].Tag.Total.Lines >> 1));
	#endif
...
//	B_Data[0] = 0;
	...
	TPI_TRACE_PRINT(("TPI_PIX_REPETITION : 0x%02X\r\n", B_Data[0]));
	
#ifdef DEV_EMBEDDED
	EMB_Status = EMB_Status;										// Dummy set
...
#endif
	B_Data[0] = ((B_Data[0] & 0xFC) | 0x02);				// HL Add
...
void SetFormat(byte *Data)
{
	...
	TPI_TRACE_PRINT((">>SetFormat() : Reg.0x09 = 0x%02X, Reg.0x0A = 0x%02X.\r\n", Data[0], Data[1]));
...
byte ChangeVideoMode(void)   
{
    ...
    Delay1ms(T_RES_CHANGE_DELAY);    	// allow control InfoFrames to pass through to the sink device.
