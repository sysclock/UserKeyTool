...
//    aByte = I2C_ReadByte(slaveID, offset);
		aByte = I2C0ReadByte(slaveID, offset, I2C_DEV_MEMORY_ADDRESS_8BIT, 1);
...
void RegisterWrite(uint16_t regAddr, uint8_t value)
{
    ...
    //I2C_WriteByte(slaveID, offset, value);
		I2C0WriteByte(slaveID, offset, I2C_DEV_MEMORY_ADDRESS_8BIT, 1, &value);
}
...
{
    uint8_t slaveID, offset;
    uint8_t aByte, *pBtye = NULL;

    DecodeRegisterAddress(regAddr, &slaveID, &offset);

    pBtye = I2C0ReadByte(slaveID, offset, I2C_DEV_MEMORY_ADDRESS_8BIT, 1);
		aByte = *pBtye; 

    aByte &= (~mask);        //first clear all bits in mask
    aByte |= (mask & value); //then set bits from value

    //I2C_WriteByte(slaveID, offset, );
		I2C0WriteByte(slaveID, offset, I2C_DEV_MEMORY_ADDRESS_8BIT, 1, &aByte);
}
...
{
    uint8_t slaveID, offset, *pBtye = NULL;

    DecodeRegisterAddress(regAddr, &slaveID, &offset);

    //I2C_ReadBlock(slaveID, offset, buffer, length);
		pBtye = I2C0ReadByte(slaveID, offset, I2C_DEV_MEMORY_ADDRESS_8BIT, length);
		memcpy(buffer, pBtye, length);
}
...
void RegisterWriteBlock(uint16_t regAddr, uint8_t* buffer, uint8_t length)
{
    ...
    //I2C_WriteBlock(slaveID, offset, buffer, length);
		I2C0WriteByte(slaveID, offset, I2C_DEV_MEMORY_ADDRESS_8BIT, length, buffer);
}
