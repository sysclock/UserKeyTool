...
BYTE nvrC256_ReadByte	(WORD W_Addr);
BYTE nvrC256_WriteByte	(WORD W_Addr, BYTE B_Data);
WORD nvrC256_ReadWord	(WORD W_Addr);
BYTE nvrC256_WriteWord	(WORD W_Addr, WORD W_Data);
BYTE* nvrC256_ReadData	(WORD W_Addr, WORD W_Len);
BYTE nvrC256_WriteData	(WORD W_Addr, BYTE *Bp_Buf, WORD W_Len);
/****************************************************************************/
...
/****************************************************************************/
/*                                                                          */
/* FUNCTION:    BYTE nvrC256_Read(WORD W_Addr)   							*/
/*                                                                          */
...
{
	BYTE* pB_Data = NULL;
	if ((pB_Data = I2C0ReadByte(NVR_C256_DEV, W_Addr, I2C_DEV_MEMORY_ADDRESS_16BIT, 1)) == NULL)
	{
		Delay10ms(1);
		if((pB_Data = I2C0ReadByte(NVR_C256_DEV, W_Addr, I2C_DEV_MEMORY_ADDRESS_16BIT, 1)) == NULL)
		{
...
/****************************************************************************/
/*                                                                          */
/* FUNCTION:    BYTE nvrC256_ReadWord(WORD W_Addr)							*/
/*                                                                          */
...
/*
	BYTE* pB_Data = NULL;
	WORD W_Data;
	if ((pB_Data = I2C0ReadByte(NVR_C256_DEV, W_Addr, 1)) == NULL)
	{
		Delay10ms(1);
		if((pB_Data = I2C0ReadByte(NVR_C256_DEV, W_Addr, 1)) == NULL)
		{
...
//	printf("Read hi W_Addr = %04x, Data = %02x\r\n", W_Addr, *pB_Data);
	...
	if ((pB_Data = I2C0ReadByte(NVR_C256_DEV, W_Addr + 1, 1)) == NULL)
	{
		Delay10ms(1);
		if((pB_Data = I2C0ReadByte(NVR_C256_DEV, W_Addr + 1, 1)) == NULL)
		{
...
*/
	BYTE* pB_Data = NULL;
	WORD W_Data;
	if ((pB_Data = I2C0ReadByte(NVR_C256_DEV, W_Addr, I2C_DEV_MEMORY_ADDRESS_16BIT, 2)) == NULL)
	{
		Delay10ms(1);
		if((pB_Data = I2C0ReadByte(NVR_C256_DEV, W_Addr, I2C_DEV_MEMORY_ADDRESS_16BIT, 2)) == NULL)
		{
...
/****************************************************************************/
/*                                                                          */
/* FUNCTION:    BYTE nvrC256_ReadData(WORD W_Addr, BYTE *Bp_Buf, WORD W_Len)*/
/*                                                                          */
...
{
	BYTE* pB_Data = NULL;
	if ((pB_Data = I2C0ReadByte(NVR_C256_DEV, W_Addr, I2C_DEV_MEMORY_ADDRESS_16BIT, W_Len)) == NULL)
	{
		Delay10ms(1);
		if((pB_Data = I2C0ReadByte(NVR_C256_DEV, W_Addr, I2C_DEV_MEMORY_ADDRESS_16BIT, W_Len)) == NULL)
		{
...
/****************************************************************************/
/*                                                                          */
/* FUNCTION:    BYTE nvrC256_WriteByte(WORD W_Addr, BYTE B_Data)			*/
/*                                                                          */
...
BYTE nvrC256_WriteByte(WORD W_Addr, BYTE B_Data)
{
	BYTE i = 0;
	i = 0;
	while (I2C0WriteByte(NVR_C256_DEV, W_Addr, I2C_DEV_MEMORY_ADDRESS_16BIT, 1, &B_Data) == (BYTE) ERR)
	{
		Delay10ms(1);
		i++;
		if(i == 5)
...
/****************************************************************************/
/*                                                                          */
/* FUNCTION:    BYTE nvrC256_WriteWord(WORD W_Addr, BYTE W_Data)			*/
/*                                                                          */
...
//	printf("EEPROM WRITE 0x%04X = [0x%04X]\r\n", W_Addr, W_Data);
	...
	{
		i = 0;
		while (I2C0WriteByte(NVR_C256_DEV, W_Addr, I2C_DEV_MEMORY_ADDRESS_16BIT, 1, &(B_Data[0])) == (BYTE) ERR)
		{
			Delay10ms(1);
			i++;
			if(i == 5)
		...
		Delay10ms(1);
		i = 0;
		while (I2C0WriteByte(NVR_C256_DEV, W_Addr + 1, I2C_DEV_MEMORY_ADDRESS_16BIT, 1, &(B_Data[1])) == (BYTE) ERR)
		{
			Delay10ms(1);
			i++;
			if(i == 5)
	...
	{
		i = 0;
		while (I2C0WriteByte(NVR_C256_DEV, W_Addr, I2C_DEV_MEMORY_ADDRESS_16BIT, 2, B_Data) == (BYTE) ERR)
		{
			Delay10ms(1);
			i++;
			if(i == 5)
...
/****************************************************************************/
/*                                                                          */
/* FUNCTION:    BYTE nvrC256_WriteData(WORD W_Addr, BYTE *Bp_Buf, WORD W_Len)	*/
/*                                                                          */
...
BYTE nvrC256_WriteData(WORD W_Addr, BYTE *Bp_Buf, WORD W_Len)
{
	...
	{
		j = 0;
		while(I2C0WriteByte(NVR_C256_DEV, W_Addr, I2C_DEV_MEMORY_ADDRESS_16BIT, W_Len, Bp_Buf) == (BYTE) ERR)
		{
			Delay10ms(1);
			j++;
			if(j == 5)
...
//		printf("Write: Bp_Data: 0x%04x, W_TempAddr: 0x%02x\r\n", Bp_Data, W_TempAddr);
		j = 0;
		while(I2C0WriteByte(NVR_C256_DEV, W_TempAddr, I2C_DEV_MEMORY_ADDRESS_16BIT, k, Bp_Data) == (BYTE) ERR)
		{
			Delay10ms(1);
			j++;
			if(j == 5)
...
//			printf("Write: Bp_Data: 0x%04x, W_TempAddr: 0x%02x\r\n", Bp_Data, W_TempAddr);
			j = 0;
			while(I2C0WriteByte(NVR_C256_DEV, W_TempAddr, I2C_DEV_MEMORY_ADDRESS_16BIT, NVR_C256_CACHE_LEN, Bp_Data) == (BYTE) ERR)
			{
				Delay10ms(1);
				j++;
				if(j == 5)
		...
		i = (W_Len - k) % NVR_C256_CACHE_LEN;
//		printf("Write: Bp_Data: 0x%04x, W_TempAddr: 0x%02x\r\n", Bp_Data, W_TempAddr);
		while(I2C0WriteByte(NVR_C256_DEV, W_TempAddr, I2C_DEV_MEMORY_ADDRESS_16BIT, i, Bp_Data) == (BYTE) ERR)
		{
			Delay10ms(1);
			j++;
			if(j == 5)
