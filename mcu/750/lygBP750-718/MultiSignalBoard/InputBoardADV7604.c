...
//					printf("Board %d , Input %d , ADV7604 Init OK Stp_TempADV7604Status_t = 0x%X\r\n", Stp_TempDeviceADV7604->B_BoardIndex, k, Stp_TempADV7604Status_t);
	...
	InputFPGA_WriteWord(INPUT_FPGA_SPI_CS_SET, 1);		// set HPD hight (DVI_IN_HPD_CON )
	for(i = 0; i < INPUT_BOARD_MAX; i++)
	{
		BoardDVIHPDSetSwitch(i, 0);
		Delay10ms(100);
	}
	InputFPGA_WriteWord(INPUT_FPGA_SPI_CS_SET, 0);		// set HPD low (DVI_IN_HPD_CON )
...
void InputBoardADV7604ProgramEDID(BYTE B_InputSlotIndex, unsigned char EdidMode)
{
	...
	{
		#if 0
		LPC_PINCON->PINSEL1 |=  (0UL << 0);								// P0.16 DVI_IN_HPD_CON
		LPC_GPIO0->FIODIR   |=  DVI_IN_HPD_CON;
		#endif
		InputFPGA_WriteWord(INPUT_FPGA_SPI_CS_SET, 0);
		...
		for(i = 0; i < gSt_InputBoardADV7604.B_ActiveChip; i++)
		{
			...
			BoardDVIHPDSetSwitch(Stp_TempDeviceADV7604->B_BoardIndex, 0);
			#if 1
			InputFPGA_WriteWord(INPUT_FPGA_SPI_CS_SET, 1);						// set P0.16 high (DVI_IN_HPD_CON )
			Delay100ms(20);
			InputFPGA_WriteWord(INPUT_FPGA_SPI_CS_SET, 0);           	// set P0.16 low (DVI_IN_HPD_CON )
			#endif
	...
	else
	{
		...
		BoardDVIHPDSetSwitch(Stp_TempDeviceADV7604->B_BoardIndex, 0);
		Delay100ms(2);
		BoardDVIHPDSetSwitch(Stp_TempDeviceADV7604->B_BoardIndex, 1);
		#if 1
...
void InputBoardADV7604SelectInputPort(BYTE B_InputSlotIndex, BYTE InputPortIndex)
{
	...
	Stp_TempADV7604Status_t = &(Stp_TempDeviceADV7604->St_ADV7604GlobalStatus_t);
	BoardIICBusSwitch(Stp_TempDeviceADV7604->B_BoardIndex);
	ADV7604_SetRouting(Stp_TempADV7604Status_t, InputPortIndex, 0, 0);
