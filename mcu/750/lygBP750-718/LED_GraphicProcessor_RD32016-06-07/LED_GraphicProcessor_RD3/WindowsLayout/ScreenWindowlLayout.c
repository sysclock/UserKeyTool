...
void ScreenWindowLayoutTask(void)
{
	...
	if(gB_CVBSSwitchEnable == 1)
	{
		...
		if(j == TVP5158_PAL)
			W_TempData |= BIT0;
		W_TempData1 = InputFPGA_ReadWord(INPUT_FPGA_SYSTEM_CVBS_TYPE);
	...
	{
		Stp_TempInputSignal = &(gStp_ProcessorInputSignal->St_InputSignalSource[i]);
		if((Stp_TempInputSignal->B_SlotIndex == 0) && (Stp_TempInputSignal->B_SignalTYPE == 1) && (gB_CVBSSwitchEnable == 1))
			W_TempData = gB_CVBSSwitchIndex;
		else
...
//		W_ScalerMask = 0;
		for(i = 0; i < gStp_ProcessorScaler->B_ActiveScalerNum; i++)
		{
			...
			if((Stp_TempScaler->B_SignalSourceSlot < 16) && (Stp_TempScaler->B_Refresh == 1))
			{
				...
				if((i < INPUT_BOARD_SCALER_MAX) && (gW_ResolutionWidth == 0) && (gW_ResolutionHeight == 0))
				{
				...
				case INPUT_DEV_VDPA:
					for(j = 0; j < gSt_InputBoardADV7604.B_ActiveChip; j++)
					{
						...
						if(Stp_TempDeviceADV7604->B_BoardIndex == B_SlotIndex)
						{
							*XStart = 0;
							*YStart = 0;
							if(InputBoardADV7604GetSignalWidthHeight(j, pResolutionWidth, pResolutionHeight) == 0xFF)
							{
								*pResolutionWidth = 0;
								*pResolutionHeight = 0;
								return -1;
							...
							*XStart = 0;
							*YStart = 0;
							BoardIICBusSwitch(Stp_TempDeviceADV7604->B_BoardIndex);
							if(ADV7604_GetSignalActiveWidthHeight(Stp_TempADV7604Status_t, pResolutionWidth, pResolutionHeight) == 0)
							{
								*pResolutionWidth = 0;
								*pResolutionHeight = 0;
								return -1;
							}
							printf("Test***1: ResolutionWidth = %d, ResolutionHeight = %d\r\n", *pResolutionWidth, *pResolutionHeight);
							*/
							return 1;
						}
					}
					for(j = 0; j < gSt_InputBoardGV7601.B_ActiveChip; j++)
					{
						...
						if(Stp_TempDeviceGV7601->B_BoardIndex == B_SlotIndex)
						{
							*XStart = 0;
							*YStart = 0;
							if(InputBoardGV7601GetSignalWidthHeight(j, pResolutionWidth, pResolutionHeight) == 0)
							{
								*pResolutionWidth = 0;
								*pResolutionHeight = 0;
								return -1;
					...
					for(j = 0; j < (gSt_InputBoardTVP5158.B_ActiveChip * 4); j++)
					{
						...
						if(Stp_TempDeviceTVP5158->B_BoardIndex == B_SlotIndex)
						{
							...
							*YStart = 0;
							if(B_SignalIndex >= 4)
							{
								if(InputBoardTVP5158GetSignalWidthHeight(0, XStart, YStart, pResolutionWidth, pResolutionHeight) == 0)
								{
									*pResolutionWidth = 0;
									*pResolutionHeight = 0;
									return -1;
							...
							{
								B_TempData = InputBoardTVP5158GetSignalWidthHeight(B_SignalIndex, XStart, YStart, pResolutionWidth, pResolutionHeight);
								if(B_TempData == 0)
								{
									*pResolutionWidth = 0;
									*pResolutionHeight = 0;
...
static int ScreenWindowLayoutCheckScreenRectInput(ST_Screen_Rect* Stp_ScreenRectInput)
{
	...
	for(i = 0; i < Stp_ScreenRectInput->ActiveRectWindowsNum; i++)
	{
		...
		if(((Stp_TempSignalSource->SignalSource < INPUT_SIGNAL_PORT_MAX) || (Stp_TempSignalSource->SignalSource == (INPUT_SIGNAL_PORT_MAX << 1))) && (gW_ResolutionWidth == 0) && (gW_ResolutionHeight == 0))
			Stp_TempSignalSource->SignalSource = INPUT_SIGNAL_PORT_MAX << 1;
		else if((Stp_TempSignalSource->SignalSource == (INPUT_SIGNAL_PORT_MAX << 1)) && (gW_ResolutionWidth != 0) && (gW_ResolutionHeight != 0))
			Stp_TempSignalSource->SignalSource = Stp_TempSignalSource->OriginalSignalSource;
		else
			Stp_TempSignalSource->SignalSource = Stp_TempSignalSource->OriginalSignalSource;
		if(Stp_TempSignalSource->SignalSource > (INPUT_SIGNAL_PORT_MAX << 1))
		{
//			Stp_TempSignalSource->SignalSource = 1;
			Stp_TempSignalSource->WindowsProperty |= BIT0;
		}
		else if (Stp_TempSignalSource->SignalSource == (INPUT_SIGNAL_PORT_MAX << 1))
...
//					InputFPGASetVideoOutputDisalble(BIT0 << B_OutputPortIndex);
			...
			if(Stp_TempSignalSource->SignalSource < INPUT_SIGNAL_PORT_MAX)
				TempData = Stp_TempSignalSource->SignalSource << 1;
			else if((Stp_TempSignalSource->SignalSource >= INPUT_SIGNAL_PORT_MAX) && (Stp_TempSignalSource->SignalSource < (INPUT_SIGNAL_PORT_MAX + INPUT_BOARD_SCALER_MAX)))
				TempData = ((Stp_TempSignalSource->SignalSource - INPUT_SIGNAL_PORT_MAX) << 1) + 1;
			else
