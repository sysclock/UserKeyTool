...
void InitInputBoardGraphicFPGA(void)
{
	...
	else
	{
		memcpy(&(St_TempSignalWin.SignalInputWindow), &St_TempRect, sizeof(ST_Rect));
		memcpy(&(St_TempSignalWin.SignalDisplayWindow), &St_TempRect, sizeof(ST_Rect));
		for(B_Data = 0; B_Data < MaxOneInputBoardScalerNum; B_Data++)
		{
...
//	ParamWrite(NVR_SCREEN_HEIGHT, g_ScreenOutputHeight);
  ...
  switch(W_TempHStart)
    ...
    case INPUT_BOARD_VDPG:
    case INPUT_BOARD_VDPH:
    case INPUT_BOARD_VDPI:
    case INPUT_BOARD_VDPJ:
      W_TempVStart = 0;
...
void InputGraphicFPGAInitAllScaler(void)
{
	...
	{
		for(B_Data = 0; B_Data < INPUT_BOARD_SCALER_MAX; B_Data++)
		{
			memset(&St_TempAperture, 0, sizeof(St_TempAperture));
			St_TempAperture.InVertSize = 1080 / 2;						// < Vertical size of the input video
			St_TempAperture.InHoriSize = 1920 / 2;						// < Horizontal size of the input video
...
//			B_Data = InputFPGASetOutputResolution(PC_BASE+28);
			...
			ParamWrite(NVR_CUSTOM_RES_ACTIVE_LINE, 768);
			printf("Input FPGA Output Timing: 1360 x 768@60Hz\r\n");
      break;
    case 5:
...
//			B_Data = InputFPGASetOutputResolution(PC_BASE+41);
      ...
      break;
    case 8:
			Set_SL38160_CLK(DEVICEID_SL38160_17, 10, 4);		// AUDIO_CLK 10--49.1520, VIDEO_CLK 4 - 1920 x 1080@30 	74.25MHz
//			B_Data = InputFPGASetOutputResolution(16);
			B_Data = InputFPGASetCustomOutputResolution(34, 2200, 1125, 0, 0, 1920, 1080);
			ParamWrite(NVR_CUSTOM_RES_ENABLE, 1);
			ParamWrite(NVR_CUSTOM_RES_CLK, 4);
			ParamWrite(NVR_CUSTOM_VIDEO_MODE_INDEX, 34);
			ParamWrite(NVR_CUSTOM_RES_TOTAL_PIXEL, 2200);
			ParamWrite(NVR_CUSTOM_RES_TOTAL_LINE, 1125);
			ParamWrite(NVR_CUSTOM_RES_H_START, 0);
			ParamWrite(NVR_CUSTOM_RES_V_START, 0);
...
//			B_Data = InputFPGASetOutputResolution(16);
      ...
      break;
    case 104:
			Set_SL38160_CLK(DEVICEID_SL38160_17, 10, 38);		// AUDIO_CLK 10--49.1520, VIDEO_CLK 37 - 2048 X 1152@60Hz 	156.750MHz
			B_Data = InputFPGASetCustomOutputResolution(PC_BASE + 51, 2208, 1185, 0, 0, 2048, 1152);
			ParamWrite(NVR_CUSTOM_RES_ENABLE, 1);
			ParamWrite(NVR_CUSTOM_RES_CLK, 38);
			ParamWrite(NVR_CUSTOM_VIDEO_MODE_INDEX, PC_BASE + 51);
			ParamWrite(NVR_CUSTOM_RES_TOTAL_PIXEL, 2208);
			ParamWrite(NVR_CUSTOM_RES_TOTAL_LINE, 1185);
			ParamWrite(NVR_CUSTOM_RES_H_START, 0);
			ParamWrite(NVR_CUSTOM_RES_V_START, 0);
			ParamWrite(NVR_CUSTOM_RES_ACTIVE_PIXEL, 2048);
			ParamWrite(NVR_CUSTOM_RES_ACTIVE_LINE, 1152);
			printf("Input FPGA Output Timing: 2048 x 1152@60Hz\r\n");
      break;
    default:
			B_Data = 0xFF;
...
BYTE InputGraphicFPGACheckChipId(void)
{
	...
	#if 0
	while((W_Data == 0xFFFFFFFF) && (B_Data < 0xFF))
	{
		Delay1ms(1);
		W_Data = InputFPGA_ReadWord(INPUT_FPGA_CHIP_ID);
		B_Data++;
