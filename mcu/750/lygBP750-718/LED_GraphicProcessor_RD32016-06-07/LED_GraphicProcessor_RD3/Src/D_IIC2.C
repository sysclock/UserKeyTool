...
#define I2C2_EXTERN_DEBUG	0

#define I2C2_MCU_PORT			GPIOA
#define I2C2_SCL_PIN			GPIO_PIN_11
#define I2C2_SDA_PIN			GPIO_PIN_12

#define SetI2C2_SCL		HAL_GPIO_WritePin(I2C2_MCU_PORT, I2C2_SCL_PIN, GPIO_PIN_SET)					// Set SCL
#define ClrI2C2_SCL		HAL_GPIO_WritePin(I2C2_MCU_PORT, I2C2_SCL_PIN, GPIO_PIN_RESET)				// Clear SCL
#define CheckI2C2_SCL	HAL_GPIO_ReadPin(I2C2_MCU_PORT, I2C2_SCL_PIN)													// Check SCL
#define SetI2C2_SDA		HAL_GPIO_WritePin(I2C2_MCU_PORT, I2C2_SDA_PIN, GPIO_PIN_SET)					// Set SDA
#define ClrI2C2_SDA		HAL_GPIO_WritePin(I2C2_MCU_PORT, I2C2_SDA_PIN, GPIO_PIN_RESET)				// Clear SDA
#define CheckI2C2_SDA	HAL_GPIO_ReadPin(I2C2_MCU_PORT, I2C2_SDA_PIN)													// Check SDA
...
uint8_t	gB_I2C2ReceiveBuf[SIO_RxBufLen] = {0};
#endif
...
#ifdef USE_SIMULATED_I2C
void I2C2_Init			(void);
void I2C2_IRQHandler		(void);				//i2c0 interrupt routine
uint8_t I2C2WriteByte(uint8_t B_I2CAddr, uint32_t W_MemAddr, uint8_t B_MemAddrType, int W_Count, uint8_t* pBuff);
uint8_t* I2C2ReadByte(uint8_t B_I2CAddr, uint32_t W_MemAddr, uint8_t B_MemAddrType, int W_Count);
#endif
...
void I2C2_Init(void)
{
  ...
  HAL_GPIO_Init(I2C2_MCU_PORT, &GPIO_InitStruct);
	memset(gB_I2C2ReceiveBuf, 0, sizeof(gB_I2C2ReceiveBuf));
	printf("I2C1 init OK!\r\n");
}
#endif
/************************************************************************************/
/*                                                                          		*/
/* FUNCTION:    uint8_t I2C2WriteByte(uint8_t B_I2CAddr, uint32_t W_MemAddr,					*/
...
/************************************************************************************/
/*                                                                          		*/
/* FUNCTION:    uint8_t* I2C2ReadByte(uint8_t B_I2CAddr, uint32_t W_MemAddr,					*/
/*									uint32_t W_Count)									*/
/*                                                                          		*/
...
static void i2c_Init(void)
{
	...
	// process of doing so.
	//
	ClrI2C2_SCL;                    // Clear SCL
	i2c2_Wait();                     // Delay

	// now send a complete i2c stop
	//
	ClrI2C2_SDA;                    // Clear SDA
	i2c2_Wait();                     // Delay
	SetI2C2_SCL;                    // Set SCL
	i2c2_Wait();                     // Delay
	SetI2C2_SDA;                    // Set SDA
...
static uint8_t i2c_Start(uint8_t B_Device) 
{
    ...
    // Generate I2C START
    //
		TimerStart		(IIC2_10ms_TMR1, 2);
    SetI2C2_SCL;                    	// Set SCL
    while(!(CheckI2C2_SCL))        	// Wait until bus is released by slave
		{
...
static uint8_t i2c_RcvAck(void) 
{
    ...
    SetI2C2_SCL;                    // Set SCL 
    i2c2_Wait();                     // Delay 
    B_Ack = CheckI2C2_SDA;          // Check SDA line 
    ClrI2C2_SCL;                    // Clear SCL 
...
static uint8_t i2c_RcvByte(void)
{
    ...
    {    
        ...
        if (CheckI2C2_SDA)						// Read data bit
