...
byte Parse861LongDescriptors(byte *);
...
//			ReadBlockEDID(EDID_BLOCK_0_OFFSET, EDID_BLOCK_SIZE, g_CommData);		// read first 128 bytes of EDID ROM
			if(gB_I2CIndex == 0)
				pData = I2C1ReadByte(EDID_ROM_ADDR, EDID_BLOCK_0_OFFSET, I2C_DEV_MEMORY_ADDRESS_8BIT, EDID_BLOCK_SIZE);
			else
				pData = I2C2ReadByte(EDID_ROM_ADDR, EDID_BLOCK_0_OFFSET, I2C_DEV_MEMORY_ADDRESS_8BIT, EDID_BLOCK_SIZE);
    ...
    do
    {
        ...
        if (Block == 1)
        {
            ...
            //ReadBlockEDID(EDID_BLOCK_1_OFFSET, EDID_BLOCK_SIZE, g_CommData);    // read first 128 bytes of EDID ROM
						if(gB_I2CIndex == 0)
							pData = I2C1ReadByte(EDID_ROM_ADDR, EDID_BLOCK_1_OFFSET, I2C_DEV_MEMORY_ADDRESS_8BIT, EDID_BLOCK_SIZE);
						else
							pData = I2C2ReadByte(EDID_ROM_ADDR, EDID_BLOCK_1_OFFSET, I2C_DEV_MEMORY_ADDRESS_8BIT, EDID_BLOCK_SIZE);
						memset((void*) g_CommData, 0, sizeof((void*) g_CommData));
						if(pData != NULL)
							memcpy(g_CommData, pData, EDID_BLOCK_SIZE);
					/*
        ...
        else
        {
            ...
            //ReadSegmentBlockEDID(Segment, Offset, EDID_BLOCK_SIZE, g_CommData);     // read next 128 bytes of EDID ROM
						if(gB_I2CIndex == 0)
							pData = I2C1ReadByte(EDID_ROM_ADDR, EDID_BLOCK_1_OFFSET + Segment * EDID_BLOCK_SIZE * 2, I2C_DEV_MEMORY_ADDRESS_8BIT, EDID_BLOCK_SIZE);
						else
							pData = I2C2ReadByte(EDID_ROM_ADDR, EDID_BLOCK_1_OFFSET + Segment * EDID_BLOCK_SIZE * 2, I2C_DEV_MEMORY_ADDRESS_8BIT, EDID_BLOCK_SIZE);
						memset((void*) g_CommData, 0, sizeof((void*) g_CommData));
						if(pData != NULL)
							memcpy(g_CommData, pData, EDID_BLOCK_SIZE);
					/*
