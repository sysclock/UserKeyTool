...
BOOL ResetPara()
{
	...
	//#endif
		...
		chinesedisplay(0x94,"....");//add ... at the end of 请稍等... ...
...
/*			SendColorValue(COLORRGB,4,g_OffsetR);
		...
		case State_FactoryReset_OK:
			clearlcd();
			if(g_LanguageFlag==CHINESE)
				chinesedisplay(0x90, g_waitStr);//请稍等...
			else  if(g_LanguageFlag==ENGLISH)
			...
			if(ResetPara())
			{
				clearlcd();
				if(g_LanguageFlag==CHINESE)
					chinesedisplay(0x90, g_ResetSuccessStr);//复位成功
				else  if(g_LanguageFlag==ENGLISH)
			...
			case State_ZoomPara_Size_Value://STATE-ID=32
			{
				U8 image;          //当前通道为主还是子
				GetWhichInput(&image);
				SetZoomHVValue(HVSIZE,image);
					...
					printf("StateMachine.c L701 State_ZoomPara_Size_Value StateValue=%d \r\n",currentState); 
				#endif
				Save(DEFAULT_MODE);
			}
			break;
