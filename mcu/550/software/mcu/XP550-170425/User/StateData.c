...
BOOL CheckManagePasswordNumber()
{
	...
	//
	if(value==g_manage_password || value==TRY_ONE_EIGHT_HOURS_DEMO_KEY) //before is 536345
	{
		return __TRUE;
	}
	return __FALSE;
...
BOOL CheckPasswordNumber()
{
	...
	if((value==((value1^value2)&0x3fff)|| (value==TRY_ONE_EIGHT_HOURS_DEMO_KEY) ) &&g_UserTry_once_onoff )  
	{
		...
		SetInputSource(g_MainInput_pre,MAIN); 
		if(g_MutiWindow!=SINGLEIMAGE)
			SetInputSource(g_SubInput_pre,SUB); 
		g_UserTry_once_EndFlag=__FALSE;
		return __TRUE;		
	}	
	...
	else if((value==168)&&(g_UserTry_168Demo_onoff))//单次使用试用的的时候，不能再用168演示
	{
		time_num=RTC_GetCounter()+g_Cmd54_168Demo_TimeLength*60;
		g_UserTry_once_end_time[0]=(time_num>>24)&0xff;  
		g_UserTry_once_end_time[1]=(time_num>>16)&0xff; 
		g_UserTry_once_end_time[2]=(time_num>>8)&0xff; 
		g_UserTry_once_end_time[3]=(time_num)&0xff; 
...
{
	U32 value,valueID,value1,value2;
	if(Number1Idx==0)
		return __FALSE;	
	valueID=ChipUniqueID%10000;
	value=atoi(Number1,Number1Idx);
	...
	if(value==((value1^value2)&0x3fff) || value==53634502)		//每一个芯片的万能密码
	{
...
{	
	...
	if(currentState==State_DebugMode_Start)
	{
		switch(value)
		{
			...
			case 61://close log recording function
				g_EnableRecordLog=0;
				Write_EEPROM_Byte(E2ROM_EnableRecordLog_ADDR,g_EnableRecordLog);
			...
			case 62:	//clear the record
					...
					LcdDisplaySetOK_ReturnMain();				
				break;
			case 63:	//input minu reocord minutes 设置最小记录时间长度
				//#ifdef LOG_Without_AuthorizationKEY
			...
			//arrear machine,trial machine authorization key
			case 80://施工方设置试用密码
...
void display_TrialTimeIs_nHours(U8 displayLCDAddr)
				{
					U8 display_char[3];
					chinesedisplay(displayLCDAddr, g_OnceTimeLongStr);					
					display_char[0]=(g_UserTry_once_time/10)+48;
					display_char[1]=(g_UserTry_once_time%10)+48;
					display_char[2]=0;	
					chinesedisplay(displayLCDAddr+4, display_char);
					}
...
{
	U8 display_char[3];
	chinesedisplay(displayLCDAddr, g_ShowTimeLongStr);					
	display_char[0]=(g_Cmd54_168Demo_TimeLength/10)+48;
	display_char[1]=(g_Cmd54_168Demo_TimeLength%10)+48;
	display_char[2]=0;	
	chinesedisplay(displayLCDAddr+4, display_char);
}	
	...
	{	case State_NewMode_Set_HSize:
		...
		{		
				if(value>60)// || value<1 )
				{
					...
					{
					clearlcd();
					chinesedisplay(0x85, " OK~ ");  //8个空格
					}
				g_Cmd54_168Demo_TimeLength=value;    
				Write_EEPROM_Byte(E2ROM_SHOW_TIMELONG_ADDR,g_Cmd54_168Demo_TimeLength);		  
				if(g_UserTry_168Demo_onoff==1)//开启
				{
					time_num=RTC_GetCounter()+g_Cmd54_168Demo_TimeLength*60;
					g_UserTry_once_end_time[0]=(time_num>>24)&0xff;  
					g_UserTry_once_end_time[1]=(time_num>>16)&0xff; 
		...
		case State_UserTry_OnceTime:
		{
			if(value>24 || value<1 )
				...
				g_UserTry_once_time=value;                        
				Write_EEPROM_Byte(E2ROM_ONCE_TIMELONG_ADDR,g_UserTry_once_time);	
				if(g_UserTry_once_onoff==1 && g_UserTry_168Demo_onoff==0) //如果演示没有打开，那么计时以单次计算，否则以演示为准
				{
					time_num=RTC_GetCounter()+g_UserTry_once_time*60*60;
					g_UserTry_once_end_time[0]=(time_num>>24)&0xff;  
					g_UserTry_once_end_time[1]=(time_num>>16)&0xff; 
		...
		case State_UserTry_Once_Password100:		
		{
			...
			if(g_CurrentState==State_Cmd51_OnOff_TryOnce)
			{
				...
				else
				{
					...
					display_TrialTimeIs_nHours(0x92);
					/*
					...
					*/
					...
					if(g_UserTry_168Demo_onoff==0) //如果演示没有打开，那么计时以单次计算，否则以演示为准
					{
						time_num=RTC_GetCounter()+g_UserTry_once_time*60*60;
						g_UserTry_once_end_time[0]=(time_num>>24)&0xff;  
						g_UserTry_once_end_time[1]=(time_num>>16)&0xff; 
			...
			//168演示密码打开或关闭
			else if(g_CurrentState==State_Cmd52_OnOff_168Demo)
			{
				if(g_UserTry_168Demo_onoff)
				{
					g_UserTry_168Demo_onoff=0; //关闭
					clearlcd();
				...
				display_DemoTimeIs_nMinutes(0x92);//on and off also display the minutes number
					
				Write_EEPROM_Byte(E2ROM_SHOW_ONOFF_ADDR,g_UserTry_168Demo_onoff);	
			}
			else if(g_CurrentState==State_Cmd54_DemoTimeLength)
			{
				//clearlcd();//g_ShowTimeLongStr				
				//display_DemoTimeIs_nMinutes(0x92);//on and off also display the minutes numbe
				ReturnMainWinDontChangeState();
			
				}
			else if(g_CurrentState==State_Cmd55_SetTryOnce_nHour)
			{
				//clearlcd();//g_ShowTimeLongStr
...
{	
	...
	else
	{
		...
		*/if(g_ScreenHSize<=1024&&g_ScreenVSize<=768)
		{
			g_OutputFormatIndex=INDEX1024X768;
		}
