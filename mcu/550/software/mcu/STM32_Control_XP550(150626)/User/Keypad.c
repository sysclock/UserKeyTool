...
//	g_OKCancelLedFlashingFlag=__FALSE;
	...
	g_FadeTime=2;
	g_Fli32626HRevision=0;
	g_Fli32626LRevision=0;
...
#if(MATHINE_TYPE==XP_520||INPUT_BOARD==SDI_INPUT)
			inputSrc1[0]='S';
			inputSrc1[1]='D';
			inputSrc1[2]='I';
			inputSrc1[3]='\0';
...
#if(MATHINE_TYPE==XP_530)
	static int adv7441_input=-1;
	if(input==INPUTSOURCE_SDI&&gAD7441_InputPort!=g_Adv7441CurrentInput)
...
void ScanKeypad(void)  //scan keypad
{
	...
	for(i=0;i<KEY_NUM;i++)    
	{
		...
		else  //此处是没有按按键
		{
			...
			{//ten minutes
				if(LeisureCount==(g_LockKeypadTime*0x2773B9D))  //大约一分钟13秒0x3000000
				{
...
//					if(!DealStepKey())
				...
				else
				{
					...
					default:
						...
						case VGA_KEY:
							SetInputSourceSwitch(NumKeyToInputSrc(keyVal));
/*							if(g_HadAdcCalibrationFlag==__FALSE)
							{
								g_HadAdcCalibrationFlag=__TRUE;
								Write_EEPROM_Byte(E2ROM_ADC_CABLIBRATION_ADDR,g_HadAdcCalibrationFlag);
...
//							break;
						...
						case NUM6_KEY:
							...
							g_Adv7441CurrentInput=DEVICE_PORT_CVBS1;
						...
						case NUM7_KEY:
						#if(INPUT_BOARD==ADV7441A_INPUT)
							g_Adv7441CurrentInput=DEVICE_PORT_CVBS2;
							Write_EEPROM_Byte(E2ROM_ADV7441A_INPUT_ADDR, (U8)g_Adv7441CurrentInput);
							SetInputSourceSwitch(NumKeyToInputSrc(keyVal));
						#endif
							break;		
						case NUM8_KEY:
						#if(INPUT_BOARD==ADV7441A_INPUT)
							g_Adv7441CurrentInput=DEVICE_PORT_VGA;
							Write_EEPROM_Byte(E2ROM_ADV7441A_INPUT_ADDR, (U8)g_Adv7441CurrentInput);
							SetInputSourceSwitch(NumKeyToInputSrc(keyVal));
						#endif
							break;		
						case NUM9_KEY:
							...
							break;		
						case NUM0_KEY:							
							g_Adv7441CurrentInput=DEVICE_PORT_HDMI;
							Write_EEPROM_Byte(E2ROM_ADV7441A_INPUT_ADDR, (U8)g_Adv7441CurrentInput);
							SetInputSourceSwitch(NumKeyToInputSrc(keyVal));
							break;					
					#endif
...
//				DealVolumeKey();
	...
	if((KeyACC==LOGNTIME&&keyPress==SWAP_KEY)
	#ifdef USE_INFRARED_IR
