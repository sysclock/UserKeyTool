...
#endif
		, ENABLE);
	GPIO_PinRemapConfig(GPIO_Remap_SWJ_JTAGDisable, ENABLE); //将IO 重映射到PB3,PB4,PA15
...
/*	GPIO_InitStructure.GPIO_Pin =VGA_SEL_H_PIN;   
	...
	*/
	...
	GPIO_Init(GPIOB, &GPIO_InitStructure);	  
/*	
	GPIO_InitStructure.GPIO_Pin = IO_UART_TX_PIN;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(IO_UART_TX_GPIO, &GPIO_InitStructure);
