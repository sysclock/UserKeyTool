...
static unsigned char g_PcTcpPort[2];
static unsigned char g_PcMacAddr[6];
static unsigned char g_PcIpAddr[4];
static unsigned char g_PcSeqVal[4];
static unsigned char g_PcSeqackVal[4];
...
void make_eth(unsigned char *buf)
{
    ...
    while(i<6)
    {
	 g_PcMacAddr[i]=buf[ETH_SRC_MAC +i];
...
void make_PcEth(unsigned char *buf)
{
    ...
    while(i<6)
    {
        buf[ETH_DST_MAC +i]=g_PcMacAddr[i];
        buf[ETH_SRC_MAC +i]=macaddr[i];
        i++;
    }
}
...
void make_PcPort(unsigned char *buf)
{
    ...
    while(i<2)
    {
        buf[TCP_DST_PORT_H_P+i]=g_PcTcpPort[i];
        i++;
    }
...
void make_PcSeqAndSeqack(unsigned char *buf, U32 number)
{
	static unsigned char PcSeqVal[4]={0};
	unsigned char i;
	...
	while(i>0)
	{
		if(PcSeqVal[i-1]!= g_PcSeqVal[i-1])
		{
			PcSeqVal[i-1]=g_PcSeqVal[i-1];
			flag=1;   //服务器应答客户端的第一帧
		}
		i--;
	}
	...
	while(i>0)
	{
		if(flag==0)  
		{
			rel_ack_num=g_PcSeqVal[i-1]+rel_ack_num;
			g_PcSeqVal[i-1]=0xff&rel_ack_num;
			PcSeqVal[i-1]=g_PcSeqVal[i-1];
			rel_ack_num=rel_ack_num>>8;
		}
		buf[TCP_SEQ_H_P+i-1]=g_PcSeqVal[i-1];
		buf[TCP_SEQACK_H_P+i-1]= g_PcSeqackVal[i-1];
		i--;
	}
}
...
void make_tcphead(unsigned char *buf,unsigned  int rel_ack_num,unsigned char mss,unsigned char cp_seq)
{
    ...
    while(i<2)
    {
	 g_PcTcpPort[i]=buf[TCP_SRC_PORT_H_P+i];
    ...
    while(i>0)
    {
	 ...
	 g_PcSeqackVal[i-1]=0xff&rel_ack_num;
        if (cp_seq)
        {
...
void fill_tcp_data_pp(unsigned char *buf,unsigned  int num, const unsigned char *progmem_s)
{
    // fill in tcp data at position pos
    int pos=0;
    // with no options the data starts after the checksum + 2 more bytes (urgent ptr)
    while (pos<num) 
    {
        buf[TCP_CHECKSUM_L_P+3+pos]=pgm_read_byte(progmem_s++);
        pos++;
    }
}
