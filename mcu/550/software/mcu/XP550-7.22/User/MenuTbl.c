...
//LCD Display Name of Which Image @ 0x97 position
	...
	if(g_MutiWindow==PBPIMAGE||g_MutiWindow==PIPIMAGE)
	{
		if(g_ActiveImageValue==MAIN)
		{
			if(g_LanguageFlag==CHINESE)
				...
				chinesedisplay(SELIMGADDR, g_MainImageEn);//SELIMG:select image
		}
...
void FullZoomVideo(U8 MainOrSub)
{
	...
	Write_EEPROM_PcPart();//compare to work good version,this sentense is additional.write to eeprom graphic channel's part or full 
}
...
//如果在全屏状态下做截取，按局部键在全屏和截取之间做局部全屏，截取画面不能保存
//在局部状态下做截取，按局部键在局部和截取放大之间切换，截取画面不能保存
//响应前面板局部/全屏 按键的操作函数
...
/* reference function implementation
//在点对点和截取放大之间做局部全屏
void FullZoomDVI(U8 MainOrSub)
{
	g_CurrentState=State_MainMenu_Not_Menu;
...
//After Boot, refresh image status:zoom, scaler or point2point?
