...
Special Note:
	1, Uart-3 commnuication with MS182 input card;
...
{	ChipUniqueID = *(__IO u32 *)(0X1FFFF7E8); }
...
{	
	...
	#ifdef DEBUG
	printf("main.c L114-load eep<  >check wizard  g_CurrentState=%d \r\n",g_CurrentState);
	#endif
	FlashReadLogo();	
		
	//向导设置或调用，调用模式前等待图像稳定
	//第一次设置g_CurrentState的值State_MainMenu_Menu=0，NewState_Set=137
	WizardOrRecoverMode();
	...
	RTC_Init();  
	#ifdef DEBUG
	printf("main.c L128-after RTC  g_CurrentState=%d \r\n",g_CurrentState);
	#endif
	...
	if(g_UserTry_168Demo_onoff ||g_UserTry_once_onoff)  //单次或者演示开启，当前时间大于截止时间，表明授权已超时
	{
	...
	{ 	
		...
		Check_TryDemoMachine_andEchoHumanOperation(n++);
	}
...
void Load_Para_TryOne_orDemo_Func(void)
{
		
	//20170213 lyg
	Read_EEPROM_Byte(E2ROM_SHOW_ONOFF_ADDR ,&g_UserTry_168Demo_onoff);
	if(g_UserTry_168Demo_onoff>1) //非正常设置状态，正常设置状态0表示关闭，1表示打开
	{
		g_UserTry_168Demo_onoff=0;
		Write_EEPROM_Byte(E2ROM_SHOW_ONOFF_ADDR,g_UserTry_168Demo_onoff);	
	}
	
	Read_EEPROM_Byte(E2ROM_SHOW_TIMELONG_ADDR ,&g_Cmd54_168Demo_TimeLength);
	if(g_Cmd54_168Demo_TimeLength>60)//// ||g_Cmd54_168Demo_TimeLength<10)  //演示时间最少1分钟，最多60分钟
	{
		g_Cmd54_168Demo_TimeLength=3;    
		Write_EEPROM_Byte(E2ROM_SHOW_TIMELONG_ADDR,g_Cmd54_168Demo_TimeLength);		     //默认演示时间为3分钟
	}
...
//	U8 image=MAIN;
	...
	{
		g_CurrentState=State_MainMenu_Menu;//state value =0
		clearlcd(); //清除LCD
		if(g_LanguageFlag==CHINESE)
		{
...
void Check_TryDemoMachine_andEchoHumanOperation(unsigned short n)
{
		#ifdef DEBUG	
		//翻看菜单，可以打印出所有状态的值
		//printf("main.c L170-in while  No.%d,g_CurrentState=%d \r\n",n,g_CurrentState);	
		#endif
