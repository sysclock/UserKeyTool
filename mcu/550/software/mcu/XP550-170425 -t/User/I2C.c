...
BOOL FindE2PROMAddr(int *addr1,int *addr2,U8 Mode)
{
	...
	switch(Mode)
	{
		...
		case MODE9:
			...
			break;
		case MODE10:
			eepromaddr1=(10*EEPROMU8_SIZE+9*EEPROMU16_SIZE*2)+E2ROM_SAVEMODE_BEGIN_ADDR;
			eepromaddr2=(11*EEPROMU8_SIZE+9*EEPROMU16_SIZE*2)+E2ROM_SAVEMODE_BEGIN_ADDR;
			break;	
		default:
			return __FALSE;
	}
...
//write the fullorpart para into eep
void Write_EEPROM_PcPart()
{
	...
	cch[3]=g_VGA2FullOrPartValue;
	cch[4]=g_DVI2FullOrPartValue;
	m24xx_write(E2ROM_FULL_PART_ADDR,cch,6);
	#ifdef DEBUG_ZOOM_YJL
	printf("i2c.c L2031 after write eep g_HDMIFullOrPartValue=%d \r\n",g_HDMIFullOrPartValue);
	#endif
}
...
BOOL Read_EEPROM_PcPart()
{
	U8 cch[6];
	int rc;
	rc=m24xx_read(E2ROM_FULL_PART_ADDR,cch,6);
	if(rc)
	{
		g_VGAFullOrPartValue=cch[0];
		g_DVIFullOrPartValue=cch[1];
		g_HDMIFullOrPartValue=cch[2];
		g_VGA2FullOrPartValue=cch[3];
		g_DVI2FullOrPartValue=cch[4];
	}
	#ifdef DEBUG_ZOOM_YJL
	printf("i2c.c L2051 after read eep g_HDMIFullOrPartValue=%d \r\n",g_HDMIFullOrPartValue);
	#endif
	return rc;
}
